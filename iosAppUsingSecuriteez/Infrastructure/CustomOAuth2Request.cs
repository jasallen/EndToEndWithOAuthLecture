...
#else
    public class CustomOAuth2Request : Request
#endif
    {
        ...
        public CustomOAuth2Request(string method, Uri url, IDictionary<string, string> parameters, Account account, bool useHeaderForToken = false)
        ...
        {
            var baseUrl = base.GetPreparedUrl();

            ValidateAccount(Account, baseUrl);

            if (_useHeaderForToken)
                return baseUrl;

            //if not using header, must get queryString
            return GetAuthenticatedUrl(Account, baseUrl, AccessTokenParameterName);
        }
